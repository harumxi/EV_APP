...
async function performSearch() {
  ...
  document.getElementById('results-list').innerHTML =
    `<div class="p-4 text-center text-gray-500"><i class="fa-solid fa-circle-notch fa-spin"></i> Searching location...</div>`;
...
async function fetchChargers(lat, lon) {
  document.getElementById('results-list').innerHTML =
    `<div class="p-4 text-center text-gray-500"><i class="fa-solid fa-bolt fa-bounce"></i> Finding hubs...</div>`;
      ...
      <div class="flex-1">
        <div class="flex justify-between items-start">
          ...
          <div class="flex items-center gap-2">
            <button onclick="event.stopPropagation(); toggleFavorite('${st.id}', '${st.name.replace(/'/g, "\\'")}', '${st.address.town || ''}')" class="hover:scale-110 transition">
...
async function loadReviews(stationId) {
  ...
  try {
    ...
    list.innerHTML = '<div class="text-center text-red-400">Failed to load reviews.</div>';
  }
}
...
function openDetailsModal(st) {
  ...
  const content = `
    <div class="space-y-6">
      ...
      <button onclick="triggerNavigation('${st.name.replace(/'/g, "\\'")}', ${st.location.latitude}, ${st.location.longitude})"
        class="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 transition transform active:scale-95">
