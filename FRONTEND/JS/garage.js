...
async function loadGarage() {
  ...
  try {
    ...
    cars.forEach((car) => {
      ...
      card.className = `group relative flex flex-col rounded-[24px] border border-white/40 bg-white/20 backdrop-blur-md overflow-hidden shadow-sm transition-all duration-500 hover:bg-white/40 hover:shadow-md hover:-translate-y-0.5 mb-4 ${isActive ? 'ring-2 ring-white/60' : ''}`;
...
function openSpecs(garageId) {
  ...
  if (dialog) {
    ...
    dialog.style.setProperty('background-color', 'rgba(255, 255, 255, 0.55)', 'important');
    dialog.style.setProperty('backdrop-filter', 'blur(18px)', 'important');
    dialog.style.setProperty('-webkit-backdrop-filter', 'blur(18px)', 'important');
  }
  ...
  if (specsModal) {
    specsModal.style.backgroundColor = "rgba(0,0,0,0.15)";
    specsModal.style.backdropFilter = "blur(4px)";
  }
  ...
  if (specsModel) {
    specsModel.className = "text-sm font-black text-gray-900 leading-tight";
    specsModel.textContent = car.model_name;
  }
  if (specsYear) {
    specsYear.className = "px-1 py-0.5 rounded-md bg-gray-900/5 border border-gray-900/10 text-[6px] font-bold text-gray-500";
  ...
  specsModal.querySelectorAll('.tile').forEach(tile => {
    ...
    if (labels[1]) labels[1].className = "text-sm font-black text-gray-900 leading-none";
  });
...
async function doSearch() {
  ...
  try {
    ...
    results.forEach((r) => {
      ...
      row.className = "group relative flex items-center justify-between p-4 rounded-[20px] border border-white/40 bg-white/20 backdrop-blur-md mb-3 hover:bg-white/40 hover:shadow-md hover:-translate-y-0.5 transition-all duration-500 shadow-sm";
